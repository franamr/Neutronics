...
class NeutronTransportSolver:
    ...
    def solve(self):
        ...
        if self.bord_cond == 'dir':  
            ...
            raise ValueError("Condición de borde inexistente: debe ser 'dir', 'neu' o 'rob' ")
        ...
        for i in range(self.N_eig):
            ...
            phi1_proj.interpolate(fem.Expression(phi1, V0.element.interpolation_points()))
            self.phi1_list.append(phi1_proj)
            
            phi2_proj = fem.Function(V0)
            phi2_proj.interpolate(fem.Expression(phi2, V0.element.interpolation_points()))
            self.phi2_list.append(phi2_proj)

    def phi_norms(self, num = 0):
        phi2_norm = np.sqrt(fem.assemble_scalar(fem.form(ufl.inner(self.phi2_list[num], self.phi2_list[num]) * ufl.dx)))
        phi1_norm = np.sqrt(fem.assemble_scalar(fem.form(ufl.inner(self.phi1_list[num], self.phi1_list[num]) * ufl.dx)))
        return phi1_norm, phi2_norm
    
    def export(self, modo = 1, name = 'result'):
        if not (0 <= modo <= len(self.phi1_list)-1):
            raise ValueError(f"Índice de modo inválido: {modo}. Debe estar entre 1 y {len(self.phi1_list)}")
